...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        const Modmail = `\`\`\`${client.commands.filter(cmd => cmd.category === "Modmail").array().map(m => m.name.charAt(0).toUpperCase() + m.name.slice(1)).join(", ")}\`\`\``
        ...
        const Musi = new Discord.MessageEmbed()
        .setTitle(`🎵 Music`)
